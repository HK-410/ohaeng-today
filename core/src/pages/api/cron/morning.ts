...
const BOTS_TO_RUN = [
  { name: 'githyung', func: runGithyungBot },
];
...
export default async function handler(
  ...
  for (const bot of BOTS_TO_RUN) {
    try {
      console.log(`[API_CRON_MORNING] Running ${bot.name} bot...`);
      const result = await bot.func(isDryRun);
      results.push({ bot: bot.name, ...result });
      console.log(`[API_CRON_MORNING] Finished ${bot.name} bot.`);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'An unknown error occurred.';
      console.error(`[API_CRON_MORNING] Error executing ${bot.name} bot:`, errorMessage);
  ...
  return res.status(200).json({
    success: true,
    message: 'Morning job execution completed.',
    results,
  });
}
